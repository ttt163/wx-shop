...
import {ajaxGet, URL, toFixed2, getRTime, isandroid} from '../../public/index'
...
class Index extends Component {
    ...
    componentWillMount () {
        ...
        if (!location.query.game_id) {
            ...
            let sQuery = location.query
            let iosGameId = sQuery.game_id
            let androidGameId = sQuery.android_gameid
            let gameId = iosGameId
            if (!androidGameId) {
                gameId = iosGameId
            } else {
                if (isandroid()) {
    ...
    getIndexData (gameId) {
        ...
        ajaxGet('/api_web_game', {'lk_game_id': _gameId}, function (data) {
            if (data.code === 200) {
                ...
                if (!_data) {
                    _data = {'web_game_id': _gameId, 'web_game_name': '', 'web_game_icon': ''}
                }
                                                        ...
                                                        <div key={index} className="item" onClick={() => this.getDetailInfo({...item, 'lk_column_id': column.lk_column_id})}>
                                                            <div className="item-goods">
                                                                <div className="goods-block">
                                                                    ...
                                                                    <div className="goods-root">{item.lk_goods_quota.toLocaleLowerCase() === 'android' ? '仅限安卓' : '仅限iOS'}</div>
