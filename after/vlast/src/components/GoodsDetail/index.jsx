...
import {GOODSTYPE, URL, QUOTATYPE} from '../../public/index'
                ...
                <Col span={21}>
                    ...
                    <Row>
                        <Col span={9}>
                            ...
                            <p>{`限购平台：${!data.lk_goods_quota ? 'iOS' : goodsPlatform[data.lk_goods_quota]}`}</p>
                        </Col>
                        <Col span={8}>
                            {data.lk_goods_type === '2' ? <p>{`透传字段：${!data.lk_attch_code ? '' : data.lk_attch_code}`}</p> : <p>{`商品活动ID：${data.lk_activite_id}`}</p>}
                            {
                                data.lk_goods_type === '1' ? (
                                    <p>限购数量：{data.lk_quota_type !== 'n' ? (
                                        <span>按{QUOTATYPE[data.lk_quota_type]}限购，每{QUOTATYPE[data.lk_quota_type]}限购 <span> {data.lk_quota_count} </span> 个</span>
                                    ) : QUOTATYPE[data.lk_quota_type]}</p>
                                ) : ''
                            }
                        </Col>
