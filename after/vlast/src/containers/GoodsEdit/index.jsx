...
import { GOODSTYPE, URL, QUOTATYPE, formatDate, USERGRUOP } from '../../public/index'
...
class GoodsEdit extends Component {
    ...
    submitEdit = (e) => {
        ...
        this.props.form.validateFieldsAndScroll((error, values) => {
            ...
            if (!err) {
                ...
                // v1.4 20180105
                if (!values.lk_rush_static) {
                    sendData = {...sendData, lk_rush_static: '2', 'lk_rush_num': 0}
                } else {
                    sendData = {...sendData, lk_rush_static: '1'}
                }
                sendData = {
                    ...sendData,
    ...
    render () {
        ...
        // console.log(this.state.richHtml)
            ...
            <Form>
                <Tabs defaultActiveKey="basic" tabPosition="left">
                        ...
                        <FormItem label="商品类型" {...formItemLayout} extra="（商品创建完成后，再次编辑商品时将不能编辑商品类型，请创建商品时注意）">
                            {getFieldDecorator('lk_goods_type', {
                                ...
                                onChange: (e) => {
                                    ...
                                    if (_val === '1') {
                                        setFieldsValue({'lk_quota_type': 'r'})
                                    } else {
                                        setFieldsValue({'lk_quota_type': 'n'})
                                    }
                                }
                            })(
                    ...
                    <TabPane tab="销售信息" key="sales">
                        ...
                        <Row>
                            <Col span="6">
                                <FormItem label="限购类型" labelCol={{span: 8}} wrapperCol={{span: 12}}>
                                    {getFieldDecorator('lk_quota_type', {
                                        initialValue: this.state.update ? selectData.lk_quota_type : getFieldValue('lk_goods_type') !== '1' ? 'n' : 'r',
                                        rules: [{
                                    ...
                                    })(<Select disabled={getFieldValue('lk_goods_type') !== '1'} style={{maxWidth: '180px'}} placeholder="请选择限购类型">
                                        {
                                            Object.keys(QUOTATYPE).map((k, index) => <Option key={k}>{k === 'n' ? QUOTATYPE[k] : k === 'r' ? `按${QUOTATYPE[k]}限购` : `${QUOTATYPE[k]}限购`}</Option>)
                                        }
                                    </Select>)}
                                </FormItem>
                            </Col>
                            <Col span="8">
                                {
                                    getFieldValue('lk_quota_type') === 'n' ? '' : (
                                        <FormItem>
                                            <div>每{QUOTATYPE[getFieldValue('lk_quota_type')]}限购&nbsp;&nbsp;
                                                ...
                                                &nbsp;&nbsp;个<span>{getFieldValue('lk_quota_type') === 'w' ? '，每周一 00:00 重置' : getFieldValue('lk_quota_type') === 'd' ? '，每天 00:00 重置' : (
                                                    getFieldValue('lk_quota_type') === 'm' ? '，每月1日 00:00 重置' : ''
                                                )}</span>
                                            </div>
                                        </FormItem>
                                    )
                        ...
                        <FormItem label="限购平台" {...formItemLayout}>
                            {getFieldDecorator('lk_goods_quota', {
                                initialValue: this.state.update ? selectData.lk_goods_quota : $.cookie('gamePlatform'),
                                rules: [{
                                    required: true, message: '请选择限购平台'
                                }]
                            })(<RadioGroup size="large">
                                <RadioButton value={$.cookie('gamePlatform')}>{goodsPlatform[$.cookie('gamePlatform')]}</RadioButton>
                                {/* {
                                    Object.keys(goodsPlatform).map((item) => (
                                        <RadioButton key={item} value={item}>{goodsPlatform[item]}</RadioButton>
                                    ))
                                } */}
                            </RadioGroup>)}
                        </FormItem>
                        ...
                        <Row>
                            <Col span="6">
                                <FormItem label="用户分组" labelCol={{span: 8}} wrapperCol={{span: 14}}>
                                    {getFieldDecorator('lk_user_group', {
                                        initialValue: !this.state.update || !selectData.lk_user_group ? '1' : `${selectData.lk_user_group}`,
                                        rules: [{
                                            required: true, message: '请选择用户分组'
                                        }]
                                    })(<RadioGroup size="large">
                                        {
                                            Object.keys(USERGRUOP).map((item, index) => (<RadioButton key={item} value={item}>{USERGRUOP[item]}</RadioButton>))
                                        }
                                    </RadioGroup>)}
                                </FormItem>
                            </Col>
                            <Col span="14">
                                {
                                    getFieldValue('lk_goods_user') === '1' ? <div className="lh32">（商品设置为正式分组时，上架后对<span className="red-color">所有用户</span>可见）</div> : (
                                        <div className="lh32">（商品设置为测试分组时，上架后仅对<span className="red-color">测试组用户</span>可见）</div>
                                    )
                                }
                            </Col>
                        </Row>
                        ...
                        <Row>
                            <Col span="3">
                                <FormItem label="限售抢购" labelCol={{span: 16}} wrapperCol={{span: 8}}>
                                    {getFieldDecorator('lk_rush_static', { initialValue: !(!this.state.update || selectData.lk_rush_static !== 1), valuePropName: 'checked' })(
                                        <Switch checkedChildren="开" unCheckedChildren="关" />
                                    )}
                                </FormItem>
                            </Col>
                            {
                                !getFieldValue('lk_rush_static') ? ''
                                    : <Col span="6">
                                        <FormItem rapperCol={{'span': 24}}>
                                            <div>限售数量&nbsp;&nbsp;
                                                {getFieldDecorator('lk_rush_num', {
                                                    initialValue: this.state.update ? selectData.lk_rush_num : '',
                                                    rules: [{
                                                        pattern: /^[0-9]*$/, message: '请输入正整数'
                                                    }, {
                                                        required: true, message: '请输入限售数量'
                                                    }]
                                                })(<Input style={{maxWidth: '100px'}}/>)}
                                                &nbsp;&nbsp;个
                                            </div>
